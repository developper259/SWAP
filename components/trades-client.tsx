...
export default function TradesClient() {
  ...
  const ALL_TRADES: Trade[] = [
    ...
    },
    {
      ...
      status: 'completed',
      date: `${t('activeTrades.completed')}: Feb 28`,
    },
  ];
  ...
  return (
    <div>
      {/* Header */}
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-foreground mb-2">{t('tradesPage.title')}</h1>
        <p className="text-muted-foreground">{t('tradesPage.subtitle')}</p>
      </div>

        {/* Stats */}
        <div className="grid grid-cols-4 gap-4 mb-8">
          <div className="bg-card border border-border rounded-lg p-4 text-center">
            <p className="text-sm text-muted-foreground">{t('tradesPage.totalTrades')}</p>
            <p className="text-2xl font-bold text-foreground">{ALL_TRADES.length}</p>
          </div>
          <div className="bg-card border border-border rounded-lg p-4 text-center">
            <p className="text-sm text-muted-foreground">{t('tradesPage.pending')}</p>
            <p className="text-2xl font-bold text-yellow-600">
              {ALL_TRADES.filter((t) => t.status === 'pending').length}
            </p>
          </div>
          <div className="bg-card border border-border rounded-lg p-4 text-center">
            <p className="text-sm text-muted-foreground">{t('tradesPage.inTransit')}</p>
            <p className="text-2xl font-bold text-purple-600">
              {ALL_TRADES.filter((t) => t.status === 'transit').length}
            </p>
          </div>
          <div className="bg-card border border-border rounded-lg p-4 text-center">
            <p className="text-sm text-muted-foreground">{t('tradesPage.completed')}</p>
